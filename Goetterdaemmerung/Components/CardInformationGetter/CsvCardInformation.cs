...
public class CsvCardInformation : ICardInformationGetter
{
    ...
    {
        var cardData = _userData.GetCardData();
        if (cardData == null) throw new Exception("No Carddata as CSV found");
        var streamReader = new StreamReader(cardData);
        return streamReader.ReadToEnd();
    }

    private static string CleanStrings(string rawString)
    {
    ...
    private static string CheckIfNull(string print)
    {
        if (print.Length == 0)
        {
            return "0";
